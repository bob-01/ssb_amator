...
boolean enc_block=false, enc_flag=false, rit_flag=false, Button_flag=false, tx_flag=false, step_flag=false, rewrite_flag=false, setup_flag=false, ftone_flag=false;

uint8_t count_avr=0, Enc_state, Enc_last, step_count=3, menu_count=0, setup_count=8, xF=1, SI5351_DRIVE_CLK0, SI5351_DRIVE_CLK1, SI5351_DRIVE_CLK2, SI5351_CAPACITOR_LOAD; // 0 ..255
int8_t enc_move=0, mode=1, XTAL = 30;
...
void setup(){
  ...
  switch (SI5351_CAPACITOR_LOAD){
                             case  0: si5351.init(SI5351_CRYSTAL_LOAD_0PF, XTAL*M, Fcorr); break;
                             case  6: si5351.init(SI5351_CRYSTAL_LOAD_6PF, XTAL*M, Fcorr); break;
                             case  8: si5351.init(SI5351_CRYSTAL_LOAD_8PF, XTAL*M, Fcorr); break;
                             case  10: si5351.init(SI5351_CRYSTAL_LOAD_10PF, XTAL*M, Fcorr); break;
  }
...
void test(){
             ...
             if (digitalRead(Button_TX) == 1 && digitalRead(Button_STEP) == 1 && digitalRead(Button_RIT) == 1 && Button_flag == true)
             {
                      Button_flag = false;
             }
        ...
        if(rit_flag){
               ...
               if (setup_count == 0){ setup_count = 1;};
	            ...
	            switch (setup_count){
                             ...
                             case  13: SI5351_CAPACITOR_LOAD+=enc_move*2; if(SI5351_CAPACITOR_LOAD < 6) SI5351_CAPACITOR_LOAD = 0; if(SI5351_CAPACITOR_LOAD > 10) SI5351_CAPACITOR_LOAD = 10; break;
                    }
        }
                lcd.clear();
                lcd.setCursor(0,0);
	            switch (setup_count){
                             ...
                             case  13:   lcd.print("CAPACITOR_LOAD"); lcd.setCursor(0,1);lcd.print(SI5351_CAPACITOR_LOAD);break;
                    }
       enc_flag = false;
    }
